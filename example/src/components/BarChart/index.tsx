...
const BarChart: React.FC<BarChartInterface> = ({ values }) => {
    ...
    const getNumbers = function (contextInstance: CanvasRenderingContext2D): void {
        ...
        for (let index = minValue; index <= maxValue; index += 10) {
            contextInstance.font = "12px Arial";
            contextInstance.fillStyle = "#000";
            contextInstance.fillText(index.toString(), 0, CHART_HEIGHT - CHART_HEIGHT * index / maxValue + 10)

            // Start a new Path
            contextInstance.beginPath();
            contextInstance.moveTo(MARGIN - 5, CHART_HEIGHT - CHART_HEIGHT * index / maxValue + 5);
            contextInstance.lineTo(MARGIN + 5, CHART_HEIGHT - CHART_HEIGHT * index / maxValue + 5);
            // Draw the Path
    ...
    const drawGraphic = () => {
        if (context.current.context && canvasReference.current) {
            ...
            if (minValue < 0) {
                ...
                contextInstance.lineTo(MARGIN, CHART_HEIGHT + (-minValue * CHART_HEIGHT / maxValue + 5));
            }
    ...
    return (
        <canvas ref={canvasReference} width={500} height={800} />
    )
}
